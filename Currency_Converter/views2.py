...
cur_ = "USD"
exrates = rates
def exchange(exrates,rates,cur_,cur):
    print(exrates)
    for i in exrates:
        
        exrates[i] = exrates[i]*rates[cur_]/rates[cur]
...
def index(request):
    ...
    try:
        if((time.time()-start)>duration):
            ...
            rates = response['rates']
            exrates = rates
            exrates=exchange(exrates,rates,"USD",cur_)
    except:
        pass
    ...
    if request.GET.get('fav'):
        ...
        exrates = exchange(exrates,rates,cur_,cur)
        cur_ = cur
